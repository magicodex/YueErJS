...
(function(global, namespace) {
  ...
  if (typeof module === 'object') {
    ...
    _global[namespace] = (global[namespace] || {});
    ...
    if ((expression instanceof Array)
      ...
      for (var index = 0; index < elements.length; index++) {
        ...
        if (!YueErUtil.isNullOrUndefined(elementValue) || defaultNull) {
          this.doSetElementValue(elements, elementValue, defaultNull);
        }
      }
    } else {
  ...
  YueErData.doGetElementValue = function(element, skipNull) {
    ...
    var handler = getDataHandlerByElement(element);
  ...
  YueErData.getDataHandlerByElement = function(element) {
    ...
    if (handlerName === null || handlerName === undefined) {
  ...
  YueErData.convertExpressionToSelector = function(expression) {
    ...
    for (var index = 0; i < expressionArray.length; index++) {
